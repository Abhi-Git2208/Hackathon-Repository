...
a = int(input('ENTER THE NO. OF CAMERAS : '))
processes = []                  # We define a list that stores the processes
with cf.ProcessPoolExecutor() as exe:
    if __name__ == '__main__':
        for i in range(a):
            p = exe.submit(show_cam, i)  # Make each call of the function into a process 
            processes.append(p)          # to make all the process work concurrently   
        # p1 = exe.submit(show_cam, 0)
        # processes.append(p1)
        # p2 = exe.submit(show_cam, 1)
        # processes.append(p2)
        # p3 = exe.submit(show_cam, 2)
        # processes.append(p3)
